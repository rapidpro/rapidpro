...
app.factory 'Flow', ['$rootScope', '$window', '$http', '$timeout', '$interval', '$log', 'utils', 'Plumb', 'Revisions', 'DragHelper', ($rootScope, $window, $http, $timeout, $interval, $log, utils, Plumb, Revisions, DragHelper) ->
  ...
  new class Flow
    ...
    TEXT = 'F'  # TODO remove once all F flows become M
    MESSAGE = 'M'
    VOICE = 'V'
    SURVEY = 'S'
    USSD = 'U'

    ALL = [MESSAGE,TEXT,VOICE,SURVEY,USSD]
    NONE = []
    ALL_TEXT = [MESSAGE,TEXT,SURVEY,USSD]
    ONLINE_TEXT = [MESSAGE,TEXT,USSD]
    ...
    constructor: ->
      ...
      @actions = [
        ...
        { type:'send', name:'Send Message', verbose_name: 'Send a message to somebody else', icon: 'icon-bubble-3', message:true, filter:[MESSAGE,TEXT,VOICE] }
        ...
        { type:'api', name:'Webhook', verbose_name:'Make a call to an external server', icon: 'icon-cloud-upload', filter:[MESSAGE,TEXT,VOICE] }
        { type:'email', name:'Send Email', verbose_name: 'Send an email', icon: 'icon-bubble-3', filter:[MESSAGE,TEXT,VOICE] }
        { type:'lang', name:'Set Language', verbose_name:'Set language for contact', icon: 'icon-language', filter:ALL }
        { type:'channel', name:'Set Channel', verbose_name:'Set preferred channel', icon: 'icon-phone', filter:[MESSAGE,TEXT,VOICE] }
        { type:'flow', name:'Start Another Flow', verbose_name:'Start another flow', icon: 'icon-tree', flows:true, filter:[MESSAGE,TEXT,VOICE] }
        { type:'trigger-flow',   name:'Start Someone in a Flow', verbose_name:'Start someone else in a flow', icon: 'icon-tree', flows:true, filter:[MESSAGE,TEXT,VOICE,USSD] }
      ]

      @rulesets = [

        { type: 'wait_message', name:'Wait for Response', verbose_name: 'Wait for response', split:'message response', filter:[MESSAGE,TEXT,SURVEY] },
        ...
        # online flows
        { type: 'webhook', name:'Call Webhook', verbose_name: 'Call webhook', split:'webhook response', filter:[MESSAGE,TEXT,VOICE,USSD], rules:[
          { name: 'Success', test: { type: 'webhook_status', status: 'success'}},
          { name: 'Failure', test: { type: 'webhook_status', status: 'failure'}},
        ]},

        { type: 'resthook', name:'Call Zapier', verbose_name: 'Call Zapier', split:'zapier response', filter:[MESSAGE,TEXT,VOICE,USSD], rules:[
          { name: 'Success', test: { type: 'webhook_status', status: 'success'}},
          { name: 'Failure', test: { type: 'webhook_status', status: 'failure'}},
        ]},

        { type: 'airtime', name:'Transfer Airtime', verbose_name: 'Transfer Airtime', split: 'transfer airtime', filter:[MESSAGE,TEXT,VOICE], rules: [
