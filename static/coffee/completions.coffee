  ...
  bind: (selector, variables=null) ->
    ...
    config =
      ...
      callbacks:
        ...
        matcher: (flag, subtext) -> return @setting.ac.parser.expressionContext(subtext)

        filter: (query, data, searchKey) ->

          if query and query[0] is '('
            data = @setting.ac.completions

          subQuery = @setting.ac.parseQuery(query)
          lastIdx = subQuery.lastIndexOf('.')
          start = subQuery.substring(0, lastIdx)
          results = @setting.ac.findCompletions(subQuery, data, start, lastIdx)
        ...
        beforeInsert: (value, item) ->
          ...
          hasMore = false
          for option in @setting.ac.variables
          ...
          isFunction = false
          for option in @setting.ac.functions
