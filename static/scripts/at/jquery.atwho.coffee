...
( (factory) ->
  ...
  DEFAULT_CALLBACKS =
    ...
    before_insert: (value, $li, selectionEvent) ->
      value
  ...
  class Controller
    ...
    insert: (str) ->
      ...
      text = "#{start_str}#{str}" + space + "#{source.slice @query['end_pos'] || 0}"
  ...
  class View
    ...
    choose: (selectionEvent) ->
      $li = @$el.find ".cur"
      @context.insert @context.callbacks("before_insert").call(@context, $li.data("value"), $li, selectionEvent)
      @context.trigger "inserted", [$li, selectionEvent]
      this.hide()
