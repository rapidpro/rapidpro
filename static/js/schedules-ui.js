...
function setDatetimeValue(datetimeText, datepickerInstance, nextStart)  {
    ...
    if (nextStart) {
        ...
        datetime = moment.tz(datetimeText, 'dddd, MMMM D, YYYY [at] h:mm a', user_tz).toDate();
    }
...
$("#modal, #id-schedule, #id-trigger-schedule").on('change', "select[name=repeat_period]", function () {
...
$('#modal, #id-schedule, #id-trigger-schedule').on('click', '.weekly-repeat-options .btn-group .btn', scheduleSelection);
...
function resetStartDatetime() {
    ...
    $(".start-button").val("Schedule Poll").removeClass('btn-success').addClass('btn-primary');

    $("#schedule-next-run").show();
    $("#next-fire").hide();
    $("#start-datetime").show();
    var datetime = $("#start-datetime");
    if (datetime.val() == "Schedule for later" || date.val() == "Select a date") {
        datetime.val("");
        $("#schedule-next-run").show();
    }
    datetime.focus();
    ...
    $('#start-datetime').datetimepicker({ showButtonPanel: false, timezone: user_tz_offset, minDateTime: null });
}
...
$(document).ready(function() {
    $('input[name=start]').on('click', function() {
        ...
        if (id == 'later-option') {
            actionButton.val('Schedule').addClass('btn-primary').removeClass('btn-success btn-danger');
