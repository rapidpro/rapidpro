...
class OrgCRUDL(SmartCRUDL):
    ...
    class Menu(MenuMixin, InferOrgMixin, SmartTemplateView):
        ...
        def derive_menu(self):
            ...
            if submenu == "settings":  # pragma: no cover
                ...
                child_orgs = Org.objects.filter(
                    parent=self.org, is_active=True, plan__in=(settings.PARENT_PLAN, settings.WORKSPACE_PLAN)
                ).order_by("name")
    ...
    class Home(SpaMixin, FormaxMixin, ContentMenuMixin, InferOrgMixin, OrgPermsMixin, SmartReadView):
        ...
        def derive_formax_sections(self, formax, context):
            ...
            if not (org.parent and org.plan in (settings.PARENT_PLAN, settings.WORKSPACE_PLAN)):
