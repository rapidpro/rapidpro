...
def process_message_task(msg_id, from_mage=False, new_contact=False):
    ...
    msg = Msg.all_messages.filter(pk=msg_id, status=PENDING).select_related('org', 'contact', 'contact_urn', 'channel').first()
...
def collect_message_metrics_task():
    ...
    if not r.get(key):
        with r.lock(key, timeout=900):
            ...
            count = Msg.all_messages.filter(direction=OUTGOING, status=QUEUED).exclude(channel=None).\
            ...
            count = Msg.all_messages.filter(direction=OUTGOING, status=INITIALIZING).exclude(channel=None).exclude(topup=None).exclude(channel__channel_type='A').count()
            analytics.gauge('temba.current_outgoing_initializing', count)

            # current # of pending messages (excluding Android)
            count = Msg.all_messages.filter(direction=OUTGOING, status=PENDING).exclude(channel=None).exclude(topup=None).exclude(channel__channel_type='A').count()
            analytics.gauge('temba.current_outgoing_pending', count)

            # current # of errored messages (excluding Android)
            count = Msg.all_messages.filter(direction=OUTGOING, status=ERRORED).exclude(channel=None).exclude(topup=None).exclude(channel__channel_type='A').count()
            analytics.gauge('temba.current_outgoing_errored', count)

            # current # of android outgoing messages waiting to be sent
            count = Msg.all_messages.filter(direction=OUTGOING, status__in=[PENDING, QUEUED], channel__channel_type='A').exclude(channel=None).exclude(topup=None).count()
            analytics.gauge('temba.current_outgoing_android', count)

            # current # of pending incoming messages that haven't yet been handled
            count = Msg.all_messages.filter(direction=INCOMING, status=PENDING).exclude(channel=None).count()
...
def check_messages_task():
    ...
    if not r.get(key):
        with r.lock(key, timeout=900):
            ...
            unhandled_messages = Msg.all_messages.filter(direction=INCOMING, status=PENDING, created_on__lte=five_minutes_ago)
