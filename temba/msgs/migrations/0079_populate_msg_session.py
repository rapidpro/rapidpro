...
def do_populate(ChannelSession, Msg):
    ...
    sessions = (
        ChannelSession.objects.filter(is_active=True)
        .exclude(started_on=None)
        .exclude(ended_on=None)
        .order_by("created_on")
    )
    updated = 0
    ...
    for idx, session in enumerate(sessions):
        msgs = Msg.objects.filter(
            session=None, contact=session.contact, created_on__gte=session.started_on, created_on__lte=session.ended_on
        )
...
class Migration(migrations.Migration):
    dependencies = [("msgs", "0078_msg_session"), ("channels", "0057_remove_channelsession_parent_and_flow")]
