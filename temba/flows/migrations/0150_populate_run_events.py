...
def fill_path_and_events(run):
    ...
    if not steps:
        ...
        for step in steps:
            ...
            # sometimes paths have a null exit_uuid, make the new path look like the old in this regard
            if PATH_EXIT_UUID in old_path[-1] and old_path[-1][PATH_EXIT_UUID] is None:
                run.path[-1][PATH_EXIT_UUID] = None
...
def backfill_flowrun_events(FlowRun, Flow, Contact, FlowStep, Msg, Channel, ContactURN, Broadcast):
    ...
    for run_id_batch in chunk_list(range(highpoint, max_run_id + 1), 4000):
        with transaction.atomic():
            ...
            batch = (
                FlowRun.objects
                .filter(id__in=run_id_batch, flow__is_active=True)
...
def apply_manual():
    ...
    backfill_flowrun_events(FlowRun, Flow, Contact, FlowStep, Msg, Channel, ContactURN, Broadcast)
