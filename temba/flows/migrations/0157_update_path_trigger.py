...
----------------------------------------------------------------------
CREATE OR REPLACE FUNCTION temba_insert_flowpathrecentrun(_from_uuid UUID, _to_uuid UUID, _run_id INTEGER, _step1_uuid UUID, _step2_uuid UUID, _visited_on TIMESTAMPTZ) RETURNS VOID AS $$
  BEGIN
    INSERT INTO flows_flowpathrecentrun("from_uuid", "to_uuid", "run_id", "step1_uuid", "step2_uuid", "visited_on")
      VALUES(_from_uuid, _to_uuid, _run_id, _step1_uuid, _step2_uuid, _visited_on);
  END;
$$ LANGUAGE plpgsql;
  ...
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    ...
    LOOP
      ...
      PERFORM temba_insert_flowpathrecentrun(
        ...
        UUID(_new_path_json->p->>'node_uuid'),
        NEW.id,
        UUID(_new_path_json->(p-1)->>'step_uuid'),
        UUID(_new_path_json->p->>'step_uuid'),
        timestamptz(_new_path_json->p->>'arrived_on')
      );
      p := p + 1;
    END LOOP;
