...
class DataRetentionTest(FlowFileTest):
    ...
    def test_purge_messages(self):
        ...
        # run and message for each contact
        self.assertEquals(count, len(runs))
        self.assertEquals(count, Msg.objects.all().count())

        # answer all three questions
        for contact in contacts:
            self.send_message(flow, 'red', contact=contact)
            self.send_message(flow, 'primus', contact=contact)
            self.send_message(flow, contact.name, contact=contact)

        # 7 messages for each run, 3 in and 4 out
        self.assertEquals(count*7, Msg.objects.all().count())
        self.assertEquals(count*7, self.org._calculate_credits_used())
        self.assertEquals(count*7, FlowStep.objects.all().count())
        self.assertEquals(count*3, Value.objects.all().count())

        # now lets purge the outbound messages from our broadcast
        broadcast = Broadcast.objects.all().first()
        self.assertEquals(count, broadcast.msgs.all().count())
        broadcast.purge()

        # missing the initial message, so only 6 per contact now
        self.assertEquals(count*6, Msg.objects.all().count())

        # flow runs are still there
        self.assertEquals(count, FlowRun.objects.all().count())

        # check our recipient counts
        broadcast = Broadcast.objects.all().first()
        self.assertEquals(count, broadcast.recipient_count)
        self.assertEquals(0, broadcast.msgs.all().count())

        # one purged broadcast for our contact
        self.assertEquals(1, Broadcast.objects.filter(purged=True, contacts__in=[contacts[0]]).count())

        # three that haven't been purged yet
        self.assertEquals(3, Broadcast.objects.filter(purged=False, contacts__in=[contacts[0]]).count())

        # other non-message data should still be the same
        self.assertEquals(count*7, self.org._calculate_credits_used())
        self.assertEquals(count*7, FlowStep.objects.all().count())
        self.assertEquals(count*3, Value.objects.all().count())

        # purge all remaining broadcasts
        for broadcast in Broadcast.objects.filter(purged=False):
            broadcast.purge()

        # 3 inboud messages per contact remain
        self.assertEquals(count*3, Msg.objects.filter().count())
        self.assertEquals(0, Msg.objects.filter(direction='O').count())

        # now export our results and make things are still kosher
        workbook = self.export_flow(flow)
        entries = workbook.sheets()[0]

        # row per contact plus the header
        self.assertEquals(count+1, entries.nrows)

        # 3x3 for the fields plus 5 metadata fields
        self.assertEquals(14, entries.ncols)

        for row, contact in enumerate(contacts):
            self.assertEquals('Red', entries.cell(row+1, 5).value)
            self.assertEquals('red', entries.cell(row+1, 6).value)
            self.assertEquals('red', entries.cell(row+1, 7).value)
            self.assertEquals('Primus', entries.cell(row+1, 8).value)
            self.assertEquals('primus', entries.cell(row+1, 9).value)
            self.assertEquals('primus', entries.cell(row+1, 10).value)
            self.assertEquals('All Responses', entries.cell(row+1, 11).value)
            self.assertEquals(contact.name, entries.cell(row+1, 12).value)
            self.assertEquals(contact.name, entries.cell(row+1, 13).value)
