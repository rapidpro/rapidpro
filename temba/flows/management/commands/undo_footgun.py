...
class Command(BaseCommand):
    ...
    def handle(self, start_id: int, dry_run: bool, quiet: bool, *args, **options):
        ...
        while True:
            run_batch = list(
                start.runs.filter(id__gt=max_run_id).only("id", "contact_id", "session_id").order_by("id")[:100]
            )
            if not run_batch:
                break

            self.fix_run_batch(run_batch, undoers, dry_run)
    ...
    def fix_run_batch(self, runs: list, undoers: dict, dry_run: bool):
        ...
        for session in FlowSession.objects.filter(id__in=session_ids):
            ...
            for run in session.output_json["runs"]:
