...
class ResultTest(FlowFileTest):
    ...
    def test_field_results(self):
        ...
        gender = ContactField.get_or_create(self.org, self.admin, 'gender', label="Gender", value_type=TEXT)
        ...
        born = ContactField.get_or_create(self.org, self.admin, 'born', label="Born", value_type=DECIMAL)
        ...
        # ok, state field!
        state = ContactField.get_or_create(self.org, self.admin, 'state', label="State", value_type=STATE)
        ...
        reg_date = ContactField.get_or_create(self.org, self.admin, 'reg_date', label="Registration Date", value_type=DATETIME)
        now = timezone.now()
    ...
    def test_category_results(self):
        ...
        state = ContactField.get_or_create(self.org, self.admin, 'state', label="State", value_type=STATE)
        district = ContactField.get_or_create(self.org, self.admin, 'district', label="District", value_type=DISTRICT)
