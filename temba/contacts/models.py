...
class Contact(RequireUpdateFieldsMixin, TembaModel):
    ...
    def set_fields(self, user, fields):
        ...
        if fields_for_delete:
            with connection.cursor() as cursor:
                ...
                remove_fields = " - ".join(f"'{field_uuid}'" for field_uuid in fields_for_delete)
                cursor.execute(
                    f"UPDATE contacts_contact SET fields = fields - {remove_fields}, modified_by_id = %s, modified_on = %s WHERE id = %s",
                    [modified_by_id, modified_on, self.id],
                )
        ...
        if changed_field_keys:
            self.handle_update(fields=fields)

    def handle_update(self, attrs=(), urns=(), fields=(), group=None, is_new=False):
        """
    ...
    def handle_update_contact(self, field_keys):
        ...
        if field_keys:
            EventFire.update_events_for_contact_fields(self, keys=field_keys)
    ...
    def reevaluate_dynamic_groups(self, for_fields=None):
        """
