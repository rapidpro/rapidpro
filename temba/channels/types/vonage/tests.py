...
class VonageTypeTest(TembaTest):
    ...
    def test_claim(self, mock_update_number, mock_buy_number, mock_get_numbers, mock_create_application):
        ...
        session[VonageType.SESSION_VONAGE_API_KEY] = "key123"
        session[VonageType.SESSION_VONAGE_API_SECRET] = "sesame"
        session.save()
        ...
        self.assertEqual(channel.config[VonageType.CONFIG_VONAGE_API_KEY], "key123")
        self.assertEqual(channel.config[VonageType.CONFIG_VONAGE_API_SECRET], "sesame")
        self.assertEqual(channel.config[VonageType.CONFIG_VONAGE_APP_ID], "myappid")
        self.assertEqual(channel.config[VonageType.CONFIG_VONAGE_APP_PRIVATE_KEY], "private")
    ...
    def test_vonage_connect(self, mock_check_credentials):
        ...
        self.assertNotIn(VonageType.SESSION_VONAGE_API_KEY, self.client.session)
        self.assertNotIn(VonageType.SESSION_VONAGE_API_SECRET, self.client.session)
        ...
        self.assertIn(VonageType.SESSION_VONAGE_API_KEY, self.client.session)
        self.assertIn(VonageType.SESSION_VONAGE_API_SECRET, self.client.session)
        self.assertEqual(self.client.session[VonageType.SESSION_VONAGE_API_KEY], "key")
        self.assertEqual(self.client.session[VonageType.SESSION_VONAGE_API_SECRET], "secret")
    ...
    def test_search(self, mock_search_numbers):
        ...
        self.channel = Channel.create(
            ...
            config={VonageType.CONFIG_VONAGE_API_KEY: "1234", VonageType.CONFIG_VONAGE_API_SECRET: "secret"},
        )

        # attach a Vonage account to the session
        session = self.client.session
        session[VonageType.SESSION_VONAGE_API_KEY] = "1234"
        session[VonageType.SESSION_VONAGE_API_SECRET] = "secret"
        session.save()
    ...
    def test_deactivate(self):
        ...
        channel.config = {
            VonageType.CONFIG_VONAGE_APP_ID: "myappid",
            VonageType.CONFIG_VONAGE_API_KEY: "api_key",
            VonageType.CONFIG_VONAGE_API_SECRET: "api_secret",
            VonageType.CONFIG_VONAGE_APP_PRIVATE_KEY: "secret",
        }
