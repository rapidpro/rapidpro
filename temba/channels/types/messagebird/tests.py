...
class TwilioTypeTest(TembaTest):
    ...
    def test_claim(self):
        ...
        self.assertEqual(
            ...
            reverse("channels.types.twilio.connect"),
        )
        ...
        with patch(
            "temba.channels.types.twilio.views.ClaimView.get_twilio_client"
        ...
        with patch(
            "temba.tests.twilio.MockTwilioClient.MockAccounts.get"
        ) as mock_get:
        ...
        with patch(
            "temba.tests.twilio.MockTwilioClient.MockPhoneNumbers.list"
        ) as mock_search:
            ...
            response = self.client.post(
                search_url, {"country": "US", "pattern": "206"}
            )
            self.assertEqual(
                response.json(),
            ...
            response = self.client.post(
                search_url, {"country": "GB", "pattern": "028"}
            )
            self.assertEqual(
                response.json(),
        ...
        with patch(
            ...
            with patch(
                ...
                response = self.client.post(
                    claim_twilio,
                    dict(country="US", phone_number="12062345678"),
                )
                self.assertRedirects(
                    response, reverse("public.public_welcome") + "?success"
                )
                ...
                self.assertEqual(
                    channel_config[Channel.CONFIG_ACCOUNT_SID], "account-sid"
                )
                self.assertEqual(
                    channel_config[Channel.CONFIG_AUTH_TOKEN], "account-token"
                )
                ...
                self.assertNotIn(
                    TwilioType.SESSION_ACCOUNT_SID, self.client.session
                )
                self.assertNotIn(
                    TwilioType.SESSION_AUTH_TOKEN, self.client.session
                )
        ...
        response = self.client.get(
            reverse("channels.types.twilio.connect") + "?claim_type=foo"
        )
        self.assertEqual(302, response.status_code)
        ...
        response = self.client.get(
            reverse("channels.types.twilio.connect")
            + "?claim_type=twilio&reset_creds=reset"
        )
        self.assertEqual(200, response.status_code)
        ...
        with patch(
            ...
            mock_numbers.return_value = iter(
                [
                    MockTwilioClient.MockPhoneNumber(
        ...
        with patch(
            ...
            with patch(
                ...
                self.assertEqual(
                    mock_numbers.call_args_list[0][1], {"page_size": 1000}
                )

                # claim it
                response = self.client.post(
                    claim_twilio, dict(country="DK", phone_number="4545335500")
                )
                self.assertRedirects(
                    response, reverse("public.public_welcome") + "?success"
                )
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "temba.tests.twilio.MockTwilioClient.MockShortCodes.get"
                ) as mock_short_codes_get:
                    mock_short_codes_get.return_value = (
                        MockTwilioClient.MockShortCode("8080")
                    )
        ...
        with patch(
            ...
            self.client.post(
                reverse("channels.channel_delete", args=[twilio_channel.uuid])
            )
    ...
    def test_update(self):
        ...
        update_url = reverse(
            "channels.channel_update", args=[twilio_channel.id]
        )
        ...
        self.assertEqual(
            twilio_channel.config[Channel.CONFIG_ACCOUNT_SID], "AccountSid"
        )
        self.assertEqual(
            twilio_channel.config[Channel.CONFIG_AUTH_TOKEN], "AccountToken"
        )
        self.assertTrue(twilio_channel.check_credentials())

        with patch(
            "temba.channels.types.twilio.type.TwilioType.check_credentials"
        ) as mock_check_credentials:
            mock_check_credentials.return_value = False

            response = self.client.post(update_url, post_data)
            self.assertFormError(
                response, "form", None, "Credentials don't appear to be valid."
            )
    ...
    def test_deactivate(self):
        ...
        with patch(
            "temba.tests.twilio.MockTwilioClient.MockPhoneNumbers.update"
        ) as mock_numbers:
            mock_numbers.side_effect = TwilioRestException(
                401, "http://twilio", msg="Authentication Failure", code=20003
            )
    ...
    def test_check_credentials(self):
        self.assertTrue(
            TwilioType().check_credentials(
                {"account_sid": "AccountSid", "auth_token": "AccountToken"}
            )
        )

        with patch(
            "temba.tests.twilio.MockTwilioClient.MockAccount.fetch"
        ) as mock_fetch:
    ...
    def test_twilio_connect(self):
        with patch(
            ...
            self.assertFormError(
                response, "form", "account_token", "This field is required."
            )
            ...
            with patch(
                ...
                with patch(
                    "twilio.rest.api.v2010.account.AccountContext.fetch"
                ) as mock_account_fetch:
                    mock_get_primary.return_value = (
                        MockTwilioClient.MockAccount(
                            "Full", "PrimaryAccountToken"
                        )
                    )
                    mock_account_fetch.return_value = (
                        MockTwilioClient.MockAccount(
                            "Full", "PrimaryAccountToken"
                        )
                    )
                    ...
                    response = self.client.post(
                        connect_url, post_data, follow=True
                    )
                    self.assertEqual(
                        response.request["PATH_INFO"],
                    ...
                    self.assertEqual(
                        response.request["PATH_INFO"],
                        reverse("channels.channel_claim"),
                    )
