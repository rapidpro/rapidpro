...
class TwitterType(ChannelType):
    ...
    def get_quick_replies(self, metadata, post_body):
        ...
        quick_replies = metadata.get('quick_reply', None)

        replies = []
        if quick_replies:
            for quick_reply in quick_replies:
                replies.append(dict(label=quick_reply['title'], metadata=quick_reply['payload']))
            ...
            post_body = dict(
                event=dict(
                    ...
                    message_create=dict(
                        target=dict(
                            recipient_id=post_body['screen_name']
                        ),
                        message_data=dict(
                            text=post_body['text'],
                            quick_reply=dict(
    ...
    def send(self, channel, msg, text):
        ...
        try:
            payload = dict(screen_name=msg.urn_path, text=text)
            dm = twitter.send_direct_message(**payload)
