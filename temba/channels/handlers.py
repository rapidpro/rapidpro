    ...
    def post(self, request, *args, **kwargs):
        ...
        if to_number and call_sid and direction == 'inbound' and status == 'ringing':
            ...
            if validator.validate(url, request.POST, signature):
                ...
                contact = Contact.get_or_create_by_urns(channel.org, channel.created_by, urns=[urn], channel=channel)
...
class TelegramHandler(BaseChannelHandler):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not contact:
            ...
            contact = Contact.get_or_create_by_urns(channel.org, channel.created_by, name, urns=[urn])
...
class JioChatHandler(BaseChannelHandler):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not contact:
            ...
            contact = Contact.get_or_create_by_urns(channel.org, channel.created_by, name=contact_name,
            ...
            if 'messaging' in entry:
                ...
                for envelope in entry['messaging']:
                    if 'optin' in envelope or 'referral' in envelope:
                        ...
                        if not contact:
                            contact = Contact.get_or_create_by_urns(channel.org, channel.created_by,
                                                                    urns=[urn], channel=channel)
...
class ViberPublicHandler(BaseChannelHandler):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # user subscribed to this user, create the contact and trigger any associated triggers
            ...
            contact = Contact.get_or_create_by_urns(channel.org, channel.created_by, contact_name, urns=[urn])
            event = ChannelEvent.create(channel, urn, ChannelEvent.TYPE_NEW_CONVERSATION, timezone.now())
            event.handle()
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'receive':
                ...
                contact = Contact.get_or_create_by_urns(channel.org, channel.created_by, name=name, urns=[fcm_urn],
    ...
    def post(self, request, *args, **kwargs):
        ...
        for dm_event in dm_events:
            if dm_event['type'] == 'message_create':
                ...
                contact = Contact.get_or_create_by_urns(channel.org, channel.created_by, name=name, urns=[urn], channel=channel)
