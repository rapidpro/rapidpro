    ...
    def test_send(self):
        ...
        try:
            ...
            with patch('requests.post') as mock:
                ...
                # assert verify was set to true
                self.assertEquals(json.loads(mock.call_args[1]['data'])['text'], "Unicode. ☺")
                self.assertEquals(json.loads(mock.call_args[1]['data'])['type'], 5)
                self.assertEquals(json.loads(mock.call_args[1]['data'])['servid'], 'SERVICE-ID')
